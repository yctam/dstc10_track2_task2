...
class Metric:
    ...
    def reset(self):
        ...
        # For knowledge cluster classification task (cid: cluster ID)
        self._cid_mrr5 = 0.0
        self._cid_r1 = 0.0
        self._cid_r5 = 0.0
    ...
    def _cid_match(self, ref_knowledge, pred_cluster):
        result = []
        for pred in pred_cluster:
            matched = False
            for ref in ref_knowledge:
                if pred['clusterid'] == ref['cluster_id']:
                    matched = True
            result.append(matched)
        return result
    ...
    def _cid_reciprocal_rank(self, ref_knowledge, pred_cluster, k=5):
        relevance = self._cid_match(ref_knowledge, pred_cluster)[:k]
    ...
    def _cid_recall_at_k(self, ref_knowledge, pred_cluster, k=5):
        relevance = self._cid_match(ref_knowledge, pred_cluster)[:k]
    ...
    def update(self, ref_obj, hyp_obj):
        if ref_obj['target'] is True:
            if hyp_obj['target'] is True:
                ...
                if 'new_response' in ref_obj and 'response' in hyp_obj:
                    self._generation_bleu1 += self._bleu(ref_obj['new_response'], hyp_obj['response'], 1)
                    self._generation_bleu2 += self._bleu(ref_obj['new_response'], hyp_obj['response'], 2)
                    self._generation_bleu3 += self._bleu(ref_obj['new_response'], hyp_obj['response'], 3)
                    self._generation_bleu4 += self._bleu(ref_obj['new_response'], hyp_obj['response'], 4)
                    self._generation_meteor += self._meteor(ref_obj['new_response'], hyp_obj['response'])
                    self._generation_rouge_l += self._rouge(ref_obj['new_response'], hyp_obj['response'], 'l')
                    self._generation_rouge_1 += self._rouge(ref_obj['new_response'], hyp_obj['response'], 1)
                    self._generation_rouge_2 += self._rouge(ref_obj['new_response'], hyp_obj['response'], 2)
                elif 'response' in hyp_obj:
                ...
                if 'cluster_rank' in hyp_obj and 'cluster_id' in ref_obj['knowledge'][0]:
                    #ref_label = ref_obj['knowledge'][0]['cluster_id']
                    #hyp_label = hyp_obj['cluster_rank'][0]['clusterid']
                    #result = 1.0 if hyp_label == ref_label else 0.0
                    #self._cid_r1 += result
                    self._cid_mrr5 += self._cid_reciprocal_rank(ref_obj['knowledge'], hyp_obj['cluster_rank'], 5)
                    self._cid_r1 += self._cid_recall_at_k(ref_obj['knowledge'], hyp_obj['cluster_rank'], 1)
                    self._cid_r5 += self._cid_recall_at_k(ref_obj['knowledge'], hyp_obj['cluster_rank'], 5)
            else:
    ...
    def scores(self):
        ...
        cid_mrr5_p, cid_mrr5_r, cid_mrr5_f = self._compute(self._cid_mrr5)
        cid_r1_p, cid_r1_r, cid_r1_f = self._compute(self._cid_r1)
        cid_r5_p, cid_r5_r, cid_r5_f = self._compute(self._cid_r5)
        ...
        scores = {
            'detection': {
                ...
                'rec': detection_r,
                'f1': detection_f,
                'tp': self._detection_tp,
                'fp': self._detection_fp,
                'tn': self._detection_tn,
                'fn': self._detection_fn
            },
            'cid': {
                'cid-mrr@5': cid_mrr5_f,
                'cid-r@1': cid_r1_f,
                'cid-r@5': cid_r5_f,
            },
            'selection': {
...
def main(argv):
    ...
    parser.add_argument('--dataset', dest='dataset', action='store', metavar='DATASET', choices=['train', 'val', 'test', 'dstc10val', 'dstc10val_clean', 'dstc10val_tl1', 'dstc10val_subset', 'dstc10val_dev50', 'dstc10val_train50', 'dstc10val_train50_c127', 'dstc10val_dev50_c127'], required=True, help='The dataset to analyze')
